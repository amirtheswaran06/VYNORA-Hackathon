    ...
    <header className="bg-[#0B1F3A] text-white sticky top-0 z-50 shadow-xl border-b border-white/10">
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-8 h-16 flex items-center justify-between gap-3">
        ...
        <div className="flex items-center gap-3 sm:gap-4">
          ...
          <div className="hidden sm:flex items-center gap-2 px-2.5 py-1 bg-[#138808] rounded-md text-[10px] font-black uppercase tracking-widest text-white shadow-sm">
        ...
        <div className="flex items-center gap-2 sm:gap-4">
          ...
          <div className="hidden md:flex items-center gap-2 text-xs font-bold">
            <span className="text-white/40 uppercase tracking-wider text-[10px] font-black">LANG:</span>
          ...
          >
            ...
            <span className="hidden lg:inline uppercase tracking-wider text-[11px]">{t('start_demo_tour', currentLang)}</span>
          ...
          >
            ...
            <span className="hidden sm:inline font-bold uppercase tracking-wider text-[10px]">IVR HUB</span>
          </button>
          ...
          <div className="relative">
            ...
            >
              ...
              {networkMode === '2g_rural' && <Radio className="w-3.5 h-3.5 text-amber-400" />}
              {networkMode === 'offline' && <WifiOff className="w-3.5 h-3.5 text-rose-400" />}
              <span className="text-[10px] hidden sm:inline font-black">
                {networkMode === 'online' ? 'Online' : networkMode === '2g_rural' ? '2G Low' : 'Offline'}
              </span>
            </button>
            ...
            {showNetworkMenu && (
              <div className="absolute right-0 mt-2 w-64 bg-[#0B1F3A] border border-white/20 rounded-2xl shadow-2xl p-2.5 z-50 text-xs text-white">
                ...
                <button
                  onClick={() => { onChangeNetworkMode('online'); setShowNetworkMenu(false); }}
                  className={`w-full text-left px-3 py-2 rounded-xl flex items-center justify-between transition mt-1 ${
                    networkMode === 'online' ? 'bg-[#1E3A5F] text-emerald-400 font-bold' : 'text-slate-300 hover:bg-white/5'
                  }`}
                >
                  ...
                  {networkMode === 'online' && <CheckCircle2 className="w-3.5 h-3.5" />}
                </button>

                <button
                  onClick={() => { onChangeNetworkMode('2g_rural'); setShowNetworkMenu(false); }}
                  className={`w-full text-left px-3 py-2 rounded-xl flex items-center justify-between transition mt-1 ${
                    networkMode === '2g_rural' ? 'bg-[#1E3A5F] text-amber-300 font-bold' : 'text-slate-300 hover:bg-white/5'
                  }`}
                >
                  <span className="flex items-center gap-2">
                  ...
                  {networkMode === '2g_rural' && <CheckCircle2 className="w-3.5 h-3.5" />}
                </button>

                <button
                  onClick={() => { onChangeNetworkMode('offline'); setShowNetworkMenu(false); }}
                  className={`w-full text-left px-3 py-2 rounded-xl flex items-center justify-between transition mt-1 ${
                    networkMode === 'offline' ? 'bg-[#1E3A5F] text-rose-300 font-bold' : 'text-slate-300 hover:bg-white/5'
                  }`}
                >
                  <span className="flex items-center gap-2">
                  ...
                  {networkMode === 'offline' && <CheckCircle2 className="w-3.5 h-3.5" />}
                </button>

                {pendingSyncCount > 0 && (
                  <div className="mt-2 pt-2 border-t border-white/10">
                    <button
                      onClick={() => { onSyncOfflineQueue(); setShowNetworkMenu(false); }}
          ...
          <div className="relative md:hidden">
            ...
            >
              ...
              <span className="font-black uppercase text-[10px]">{currentLang}</span>
            </button>
            ...
            {showLangDropdown && (
              <div className="absolute right-0 mt-2 w-40 bg-[#0B1F3A] border border-white/20 rounded-2xl shadow-2xl p-1.5 z-50 text-xs">
                {SUPPORTED_LANGUAGES.map((lang) => (
                  <button
                    key={lang.code}
                    onClick={() => { onSelectLang(lang.code); setShowLangDropdown(false); }}
                    className={`w-full text-left px-2.5 py-1.5 rounded-xl flex items-center justify-between transition ${
                      currentLang === lang.code ? 'bg-[#FF9933] text-[#0B1F3A] font-black' : 'text-slate-200 hover:bg-white/5'
                    }`}
                  >
                    <span>{lang.nativeName}</span>
                    <span className="text-[9px] uppercase font-bold opacity-75">{lang.code}</span>
                  </button>
                ))}
              </div>
            )}
            ...
            {showNotifs && (
              <div className="absolute right-0 mt-2 w-72 sm:w-80 bg-[#0B1F3A] border border-white/20 rounded-2xl shadow-2xl p-3 z-50 text-xs text-slate-200">
                <div className="flex items-center justify-between pb-2 border-b border-white/10">
                  <span className="font-black text-sm text-white uppercase tracking-wider">Alerts Hub</span>
                  ...
                  {notifications.map((notif) => (
                    ...
                    >
                      <div className="flex items-center justify-between font-black text-xs">
                        ...
                        <span className="text-[10px] opacity-75 font-normal">{notif.timestamp}</span>
                      </div>
                      <p className="mt-1 text-[11px] opacity-90 leading-snug">{notif.message}</p>
                    </div>
                  ))}
                </div>
              </div>
      ...
      {/* Role Navigation Switcher Bar with Bold Typography */}
        ...
        <div className="max-w-7xl mx-auto flex items-center justify-start sm:justify-center overflow-x-auto py-2 gap-2 no-scrollbar">
          {roles.map((r) => {
            ...
            return (
              ...
              >
                <Icon className={`w-3.5 h-3.5 ${isActive ? 'text-[#0B1F3A]' : 'text-white/40'}`} />
                <span>{r.label}</span>
              </button>
            );
          })}
      ...
      {networkMode !== 'online' && (
        <div className={`px-4 py-2 text-xs text-center font-bold flex items-center justify-center gap-2 ${
          networkMode === '2g_rural' ? 'bg-amber-500/20 text-amber-200 border-b border-amber-500/30' : 'bg-rose-600/30 text-rose-200 border-b border-rose-500/40'
        }`}>
          ...
          {pendingSyncCount > 0 && (
            <span className="font-black underline cursor-pointer ml-1 text-[#FF9933]" onClick={onSyncOfflineQueue}>
